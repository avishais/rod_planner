...
*link4_to_draw, *link5_to_draw, *link6_to_draw, *link7_to_draw, *ped_to_draw;

PQP_Model base2, link12, link22, link32, link42, link52, link62, link72, rod, EE, EE2;
...
double dis = 3000.0, azim = 0.0, elev = 30.0, swi = -120;//-115;
...
void DisplayCB()
{
    ...
    */
	...
	//MRotY(Mrod,-3.1415926/2); // Rotate rod to be grasped
	//MxM(Rrod,REE,Mrod);
	MxV(Trod,REE,T2);
	VpV(Trod,Trod,TEE);
...
void execute_path(int k){
	...
	if(k == 0){
		const char* rod_pfile = "../path/rod_path_Bx.txt";
		const char* robot_pfile = "../path/robot_paths_Bx.txt";
		FILE *fro, *fr;
		int i, nlines;
		...
		for (i = 0; i < nlines; i++)
		{
			...
			std::cout << "Rot52T: " << rot52T << std::endl;
