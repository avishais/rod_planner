#include "collisionDetectionABB.h"

#define CADLINK "/home/avishai/Downloads/omplapp/ompl/Workspace/precomputation/simulatorABB/"
...
/*
	...
	int qsize = 2;
	PQP_REAL tolerance = 8.0;
	PQP_DistanceResult res[80];

	// robot 1 collision
	PQP_Distance(&res[0],R0,T0,&base,R4,T4,&link4,rel_err,abs_err,qsize);
	PQP_Distance(&res[1],R1,T1,&link1,R4,T4,&link4,rel_err,abs_err,qsize);
	PQP_Distance(&res[2],R2,T2_t,&link2,R4,T4,&link4,rel_err,abs_err,qsize);
	PQP_Distance(&res[3],R0,T0,&base,R5,T5,&link5,rel_err,abs_err,qsize);
	PQP_Distance(&res[4],R1,T1,&link1,R5,T5,&link5,rel_err,abs_err,qsize);
	PQP_Distance(&res[5],R2,T2_t,&link2,R5,T5,&link5,rel_err,abs_err,qsize);
	PQP_Distance(&res[6],R0,T0,&base,R6,T6,&link6,rel_err,abs_err,qsize);
	PQP_Distance(&res[7],R1,T1,&link1,R6,T6,&link6,rel_err,abs_err,qsize);
	PQP_Distance(&res[8],R2,T2_t,&link2,R6,T6,&link6,rel_err,abs_err,qsize);
	PQP_Distance(&res[9],R0,T0,&base,R3,T3,&link3,rel_err,abs_err,qsize);
	PQP_Distance(&res[10],R0,T0,&base,R6,T7,&EE,rel_err,abs_err,qsize);
	PQP_Distance(&res[11],R1,T1,&link1,R6,T7,&EE,rel_err,abs_err,qsize);
	PQP_Distance(&res[12],R2,T2_t,&link2,R6,T7,&EE,rel_err,abs_err,qsize);

	// robot 2 collision
	PQP_Distance(&res[13],R02,T02,&base2,R42,T42,&link42,rel_err,abs_err,qsize);
	PQP_Distance(&res[14],R12,T12,&link12,R42,T42,&link42,rel_err,abs_err,qsize);
	PQP_Distance(&res[15],R22,T2_t2,&link22,R42,T42,&link42,rel_err,abs_err,qsize);
	PQP_Distance(&res[16],R02,T02,&base2,R52,T52,&link52,rel_err,abs_err,qsize);
	PQP_Distance(&res[17],R12,T12,&link12,R52,T52,&link52,rel_err,abs_err,qsize);
	PQP_Distance(&res[18],R22,T2_t2,&link22,R52,T52,&link52,rel_err,abs_err,qsize);
	PQP_Distance(&res[19],R02,T02,&base2,R62,T62,&link62,rel_err,abs_err,qsize);
	PQP_Distance(&res[20],R12,T12,&link12,R62,T62,&link62,rel_err,abs_err,qsize);
	PQP_Distance(&res[21],R22,T2_t2,&link22,R62,T62,&link62,rel_err,abs_err,qsize);
	PQP_Distance(&res[22],R02,T02,&base2,R32,T32,&link32,rel_err,abs_err,qsize);
	PQP_Distance(&res[23],R02,T02,&base2,R62,T72,&EE2,rel_err,abs_err,qsize);
	PQP_Distance(&res[24],R12,T12,&link12,R62,T72,&EE2,rel_err,abs_err,qsize);
	PQP_Distance(&res[25],R22,T2_t2,&link22,R62,T72,&EE2,rel_err,abs_err,qsize);

	// inter-robot collision  link2 and up for all robot
	PQP_Distance(&res[26],R3,T3,&link3,R32,T32,&link32,rel_err,abs_err,qsize);
	PQP_Distance(&res[27],R3,T3,&link3,R42,T42,&link42,rel_err,abs_err,qsize);
	PQP_Distance(&res[28],R3,T3,&link3,R52,T52,&link52,rel_err,abs_err,qsize);
	PQP_Distance(&res[29],R3,T3,&link3,R62,T62,&link62,rel_err,abs_err,qsize);
	PQP_Distance(&res[30],R4,T4,&link4,R32,T32,&link32,rel_err,abs_err,qsize);
	PQP_Distance(&res[31],R4,T4,&link4,R42,T42,&link42,rel_err,abs_err,qsize);
	PQP_Distance(&res[32],R4,T4,&link4,R52,T52,&link52,rel_err,abs_err,qsize);
	PQP_Distance(&res[33],R4,T4,&link4,R62,T62,&link62,rel_err,abs_err,qsize);
	PQP_Distance(&res[34],R5,T5,&link5,R32,T32,&link32,rel_err,abs_err,qsize);
	PQP_Distance(&res[35],R5,T5,&link5,R42,T42,&link42,rel_err,abs_err,qsize);
	PQP_Distance(&res[36],R5,T5,&link5,R52,T52,&link52,rel_err,abs_err,qsize);
	PQP_Distance(&res[37],R5,T5,&link5,R62,T62,&link62,rel_err,abs_err,qsize);
	PQP_Distance(&res[38],R6,T6,&link6,R32,T32,&link32,rel_err,abs_err,qsize);
	PQP_Distance(&res[39],R6,T6,&link6,R42,T42,&link42,rel_err,abs_err,qsize);
	PQP_Distance(&res[40],R6,T6,&link6,R52,T52,&link52,rel_err,abs_err,qsize);
	PQP_Distance(&res[41],R6,T6,&link6,R62,T62,&link62,rel_err,abs_err,qsize);

	//inter EE collisions
	PQP_Distance(&res[42],R3,T3,&link3,R62,T72,&EE2,rel_err,abs_err,qsize);
	PQP_Distance(&res[43],R4,T4,&link4,R62,T72,&EE2,rel_err,abs_err,qsize);
	PQP_Distance(&res[44],R5,T5,&link5,R62,T72,&EE2,rel_err,abs_err,qsize);
	PQP_Distance(&res[45],R6,T6,&link6,R62,T72,&EE2,rel_err,abs_err,qsize);
	PQP_Distance(&res[46],R6,T7,&EE,R32,T32,&link32,rel_err,abs_err,qsize);
	PQP_Distance(&res[47],R6,T7,&EE,R42,T42,&link42,rel_err,abs_err,qsize);
	PQP_Distance(&res[48],R6,T7,&EE,R52,T52,&link52,rel_err,abs_err,qsize);
	PQP_Distance(&res[49],R6,T7,&EE,R62,T62,&link62,rel_err,abs_err,qsize);

	// robot collision with rod
	PQP_Distance(&res[50],R6,T7,&rod,R0,T0,&base,rel_err,abs_err,qsize);
	PQP_Distance(&res[51],R6,T7,&rod,R1,T1,&link1,rel_err,abs_err,qsize);
	PQP_Distance(&res[52],R6,T7,&rod,R2,T2_t,&link2,rel_err,abs_err,qsize);
	PQP_Distance(&res[53],R6,T7,&rod,R3,T3,&link3,rel_err,abs_err,qsize);
	PQP_Distance(&res[54],R6,T7,&rod,R4,T4,&link4,rel_err,abs_err,qsize);
	PQP_Distance(&res[55],R6,T7,&rod,R5,T5,&link5,rel_err,abs_err,qsize);
	PQP_Distance(&res[56],R6,T7,&rod,R6,T6,&link6,rel_err,abs_err,qsize);
	PQP_Distance(&res[57],R6,T7,&rod,R02,T02,&base2,rel_err,abs_err,qsize);
	PQP_Distance(&res[58],R6,T7,&rod,R12,T12,&link12,rel_err,abs_err,qsize);
	PQP_Distance(&res[59],R6,T7,&rod,R22,T2_t2,&link22,rel_err,abs_err,qsize);
	PQP_Distance(&res[60],R6,T7,&rod,R32,T32,&link32,rel_err,abs_err,qsize);
	PQP_Distance(&res[61],R6,T7,&rod,R42,T42,&link42,rel_err,abs_err,qsize);
	PQP_Distance(&res[62],R6,T7,&rod,R52,T52,&link52,rel_err,abs_err,qsize);
	PQP_Distance(&res[63],R6,T7,&rod,R62,T62,&link62,rel_err,abs_err,qsize);

	// rod collisions with EE's with rod not counting buffer from parent EE
	PQP_Distance(&res[64],R6,T7,&rod_EE1,R6,T7,&EE,rel_err,abs_err,qsize);
	PQP_Distance(&res[65],R6,T7,&rod_EE2,R62,T72,&EE2,rel_err,abs_err,qsize);
	PQP_Distance(&res[66],R6,T7,&EE,R62,T72,&EE2,rel_err,abs_err,qsize);

	// robot collision with ground
	PQP_Distance(&res[67],R0,T0,&table,R2,T2_t,&link2,rel_err,abs_err,qsize);
	PQP_Distance(&res[68],R0,T0,&table,R3,T3,&link3,rel_err,abs_err,qsize);
	PQP_Distance(&res[69],R0,T0,&table,R4,T4,&link4,rel_err,abs_err,qsize);
	PQP_Distance(&res[70],R0,T0,&table,R5,T5,&link5,rel_err,abs_err,qsize);
	PQP_Distance(&res[71],R0,T0,&table,R6,T6,&link6,rel_err,abs_err,qsize);
	PQP_Distance(&res[72],R0,T0,&table,R6,T7,&EE,rel_err,abs_err,qsize);
	PQP_Distance(&res[73],R0,T0,&table,R2,T2_t2,&link22,rel_err,abs_err,qsize);
	PQP_Distance(&res[74],R0,T0,&table,R32,T32,&link32,rel_err,abs_err,qsize);
	PQP_Distance(&res[75],R0,T0,&table,R42,T42,&link42,rel_err,abs_err,qsize);
	PQP_Distance(&res[76],R0,T0,&table,R52,T52,&link52,rel_err,abs_err,qsize);
	PQP_Distance(&res[77],R0,T0,&table,R62,T62,&link62,rel_err,abs_err,qsize);
	PQP_Distance(&res[78],R0,T0,&table,R62,T72,&EE2,rel_err,abs_err,qsize);

	//rod collision with ground
	PQP_Distance(&res[79],R0,T0,&table,R6,T7,&rod,rel_err,abs_err,qsize);
